...
1. [x] **Camera Asset Inventory (qt_hama_gui)**
...
2. [x] **DCAM Connectivity & Streaming Test (Using Existing Assets)**
...
3. [x] **DCAM Settings Round-Trip Verification (Using Existing Assets)**
...
6. [x] **`FrameCache` Memory Management Test**
...
**Build System and Project Scaffolding**
...
14. [x] Implement `IAnalysisModule` abstract base class (`include/IAnalysisModule.h`).
    ...
    -   Done by agent14, commit aebfb85, PR 36
...
22. [x] Implement core droplet detection pipeline: Gaussian blur, adaptive threshold, morphology, contour finding.
    SIGN agent01/todo_droplet_detection_pipeline DONE task 22
...
23. [x] Implement fluorescence quantification logic: local annulus and global ROI background correction, SBR computation.
    SIGN agent05/todo_fluorescence_quantification DONE task 23
...
**Input Source Implementations (in `libdroplet`)**
...
**Unit Testing (Phase 1 Comprehensive)**

29. [ ] **Test: Geometry metrics:** Unit tests for area, circularity, diameter, major/minor axis, aspect ratio. These tests will use known synthetic contours (e.g., perfect circles, ellipses) and assert the calculated metrics are within a small tolerance of expected values.
30. [ ] **Test: Adaptive threshold and morphology:** Unit tests apply adaptive thresholding and morphological operations (open, close) to synthetic grayscale images with known features and noise. Assert that features are correctly segmented and noise is removed.
31. [ ] **Test: Tracking association and lifecycle:** Unit tests for tracking with synthetic data:
...
32. [ ] **Test: Fluorescence metrics and background correction:** Unit tests for mean, integrated, min, max, background-corrected mean, and SBR. Test various background methods (local annulus, global ROI) with synthetic images, including edge cases like annuli overlapping other droplets or being too small.
33. [ ] **Test: Auto-tune scoring and parameter selection:** Unit tests for the auto-tune quality scoring function, ensuring it correctly penalizes over-segmentation, under-detection, and irregular shapes. Repeated auto-tune runs on static synthetic images should yield consistent optimal parameters.
34. [ ] **Test: TIFF validation rules:** Unit tests for `ImageSequenceSource` and `MultiPageTIFFSource` that cover TIFF compliance (uncompressed, strip-based, grayscale, no BigTIFF, max dimensions) and format consistency across pages.
...
36. [ ] The CLI should accept: `--module`, `--input`, `--output`, and an optional `--config` (JSON file).
...
**Fluorescence & Multi-Channel Population Modules**
...
74. [ ] Implement the full export pipeline:
...
**Camera Control UI Wiring (Reuse Patterns)**
...
**Stability and Validation (Reuse)**
...
**Final Validation and Ground Truth**
